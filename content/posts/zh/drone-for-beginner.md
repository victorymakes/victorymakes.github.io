---
title: Drone 初探
description: Drone 是一个基于 Docker 的 CI/CD 工具，其所有编译、测试的流程都在 Docker 容器中进行。 其主要核心应该就是 configuration as a code + docker 了。每个项目下都需要定义一个 yml 配置文件，默认为 .drone.yml ， 在该配置文件中可...
date: 2019-06-21
tags:
  - id: "ci"
    title: "CI"
...
2. 安装 Docker CE
   ...
   5. 验证 Docker CE 是否正常启动
      ...
      ![docker验证](/posts/drone-for-beginner/docker-hello-world.png)
...
1. 创建 GitHub OAuth Apps
   ...
   ![创建Apps](/posts/drone-for-beginner/drone-github.png)
   ...
   services:
     drone-server:
       ...
       environment:
         ...
         - DRONE_SERVER_HOST=drone.victoryhub.cc
         ...
         # 配置管理员
         - DRONE_USER_CREATE=username:victorymakes,admin:true
     ...
     drone-agent:
       ...
       volumes:
         - /var/run/docker.sock:/var/run/docker.sock
       environment:
         - DRONE_RPC_SERVER=drone.victoryhub.cc
...
![Drone服务](/posts/drone-for-beginner/drone-web.png)
点击 Activate 进入即可激活。激活后如果项目中已有 .drone.yml 配置文件的话，当你 push 代码后， Drone 会自动执行你配置的
Pipelines 。上图是我已经激活并配置了 victorymakes.github.io 项目。
...
1. 配置项目 CI/CD 配置 .drone.yml
   ...
   steps:
     - name: build
       ...
       volumes:
         - name: target
           path: /drone/target/victorymakes.github.io
       commands:
         - npm install
         - ./node_modules/hexo/bin/hexo clean
         - ./node_modules/hexo/bin/hexo g
         - pwd && rm -rf /drone/target/victorymakes.github.io/*
         - mv /drone/src/public/* /drone/target/victorymakes.github.io
         - cd /drone/target/victorymakes.github.io && ls -l
...
2. 验证 CI/CD
   ...
   ![Drone任务](/posts/drone-for-beginner/drone-task.png)
   ![Drone任务详情](/posts/drone-for-beginner/drone-task-detail.png)
   ...
   steps:
     - name: build
       ...
       commands:
         ...
         - ./node_modules/hexo/bin/hexo clean
         - ./node_modules/hexo/bin/hexo g
         - rm -rf /drone/target/victorymakes.github.io/*
         # 需要将构建好的文件复制到不同的目录，供后面的 step 使用
         - cp -r /drone/src/public/* /drone/target/victorymakes.github.io
     ...
     - name: publish github pages
       ...
       settings:
         remote_url: https://github.com/victorymakes/victorymakes.github.io.git
     ...
     - name: publish coding pages
       # 这里使用的是我自己构建的镜像
       image: victorymakes/drone-gh-pages
       settings:
         # coding pages 相关配置
         machine: git.coding.net
         remote_url: https://git.coding.net/victorymakes/victorymakes.github.io.git
...
2. 添加 Secrets
   ...
   ![添加Secret](/posts/drone-for-beginner/drone-secret.png)

### 总结

从上面的操作的感受来说，Drone 的代码即配置，有点像 docker-compose ，整体比较简洁，并且支持 pipeline steps 、webhook、services
等。基本 CI/CD 的功能都是支持的，且符合云原生的趋势。但是感觉还是比较年轻啊，而且文档是真心有点简陋(比如中途想看 Drone 插件编写的相关文档就没找着)，多数时候还是需要自己去 Google 下。
另外，文档不支持搜索也是有点鸡儿疼的。
