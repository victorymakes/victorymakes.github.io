...
description: 最近自己的香港阿里云服务器到期了，也不打算续期了，是在太贵。最低配一年一千多，其实我也没啥事要做，再续期的话就不划算了，遍放弃了。但是，科学上网的需求还是有的啊。于是便想要买个 VPS 自己整个跳转用用，对于这个 VPS 我只有两个要求： 1. 便宜，一定要便宜！！！！毕竟自己需求较小，花多了钱觉得...
date: 2019-08-27
tags:
  - id: "vps"
    title: "VPS"
...
2. [10 美元以下国内 VPS/美国 VPS 推荐](https://www.vpser.net/ten-dollars-vps)
3. [老左常用国内/国外便宜 VPS 主机商家推荐整理](https://www.laozuo.org/myvps)
...
1. 他家的 VPS 主要在拉斯维加斯、纽约、卢森堡三个地方。不一定都有货，选择网络连通较好并有货的就 OK。我买的是拉斯维加斯的
...
2. 配置 VPS（半年付开始会更便宜，不过我是先买一个月试水）
   ![配置VPS](/posts/build-vpn-with-vps/config-vps.png)

3. 选择付款方式并付款
   ![付款](/posts/build-vpn-with-vps/pay-vps.png)

4. 成功购置，服务处于 Active 状态
   ![Active](/posts/build-vpn-with-vps/check-vps.png)

5. 登录 后台管理 [Stallion](https://manage.buyvm.net) ，为服务安装 CentOS
   ![安装系统](/posts/build-vpn-with-vps/install-vps.png)
...
> 2016 年 9 月，Google 开源了其 TCP BBR 拥塞控制算法，并提交到了 Linux 内核，从 4.9 开始，Linux 内核已经用上了该算法。根据以往的传统，Google 总是先在自家的生产环境上线运用后，才会将代码开源，此次也不例外。  
> 根据实地测试，在部署了最新版内核并开启了 TCP BBR 的机器上，网速甚至可以提升好几个数量级。  
> 通俗的来说，BBR 就和锐速一样，是用来提速的，当然再如何提速也不可能突破物理带宽，他只是一个优化网络拥堵的算法。
...
懒一点的也可以直接使用我的一个工具包 [docker-utils](https://github.com/victorymakes/docker-utils)  
按照说明启动对应的服务即可。  
里面的一些个人相关的配置需要自己修改下.
