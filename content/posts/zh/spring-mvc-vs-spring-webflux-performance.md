...
description: 最近工作有个小项目，其场景主要是封装内部的接口请求，然后做个转换之后，就请求外部请求，之后再 将外部响应转换成内部的统一格式，其实有点类似一个简单网关的应用，虽然也有一些业务逻辑在里面， 但是主要场景还是请求的转发处理，是一个 IO 密集型的应用，而且外部请求的延迟相对比较大而且不可控。 我想，这不...
date: 2020-07-10
tags:
  - id: "reactor"
    title: "Reactor"
...
![WebFlux](/posts/spring-mvc-vs-spring-webflux-performance/springboot2.png)
...
> 关于 I/O 模型，可以看看这篇 [Java 进阶（五）Java I/O 模型从 BIO 到 NIO 和 Reactor 模式](http://www.jasongj.com/java/nio_reactor/)
...
![MVC & WebFlux](/posts/spring-mvc-vs-spring-webflux-performance/spring-mvc-and-webflux-venn.png)
...
![MVC & WebFlux](/posts/spring-mvc-vs-spring-webflux-performance/flux-vs-mvc.png)
...
- 吞吐量
  ...
  ![吞吐量和相应时间对比](/posts/spring-mvc-vs-spring-webflux-performance/mvc-vs-flux-1000threads.png)

- 资源使用情况
  - mvc

    ![mvc](/posts/spring-mvc-vs-spring-webflux-performance/mvc-monitor.png)

  - flux

    ![flux](/posts/spring-mvc-vs-spring-webflux-performance/flux-monitor.png)

##### 延迟 20ms

- 吞吐量

  ![吞吐量和相应时间对比](/posts/spring-mvc-vs-spring-webflux-performance/mvc-vs-flux-1000threads-20ms.png)

从压测结果来看，如果是 IO 密集型的应用，使用 WebFlux 和使用 MVC 对于吞吐量的影响不大，但是
使用了 WebFlux 的应用，其整体响应时间更短，启动的线程数更少，使用的内存资源更少。同时，延迟越大，
WebFlux 的优势越明显。

上述的完整压测代码可以再 [这儿](https://github.com/victorymakes/webfluxtest) 找到。
