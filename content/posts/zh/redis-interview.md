---
title: Redis 问答 PK
description: 作为一个后端程序员，面试过程中难免会被问到缓存的一些问题，而目前来说，Redis 就是使用的最为广泛的一个缓存中间件了。下面我们就以 Redis 为例，说一说面试过程中会经常会被问到的一些面试题。并试着了解面试官所想，抓住重点，奋力一击，让面试不再烦恼。 首先，简单回顾下 Redis 的简介，并提供...
date: 2020-07-23
tags:
  - id: "redis"
    title: "Redis"
...
![Redis数据结构](/posts/redis-interview/xmind.png)

#### 为什么要使用 Redis 缓存？

解读：这个问题可回答空间比较广，属于 **WHY** 和 **HOW** 的一个结合体。面试官在这里其实更多的是一个基本考核，想知道这个应聘者是否会有自己思想，会对项目有个思考，而不是傻傻就只知道：

![Fuck and copy](/posts/redis-interview/fuck-and-copy.png)
